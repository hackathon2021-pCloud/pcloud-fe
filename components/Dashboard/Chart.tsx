...
const convertValue = (value) => `${Math.floor(value / 10e2)}`;
const convertDate = (timestamp) => {
  return formatDate(Number(timestamp), "dd MMM");
};
const BASE_SIZE_PER_CLUSTER = 500;
const CHECKPOING_SIZE_PER_CLUSTER = 2;
...
export default function SimpleChart() {
  ...
  const chartData = useMemo(() => {
    ...
    const randomInt = randomIntGenerator(
      user.sub
        .split("")
        .reduce((res, current) => res + current.charCodeAt(0), 0)
    );
